...
void CDrawUtilities::DrawEntity(u32 clr, ref_shader s)
{
	...
	// render flagshtok
	DU_DRAW_SH		(dxRenderDeviceRender::Instance().ShaderTransform);
...
{
    if (bWire){
        DU_DRAW_SH_C		(dxRenderDeviceRender::Instance().ShaderTransform, clr_w);
		m_WireCone.Render	();
    }
    if (bSolid){
        DU_DRAW_SH_C		(color_get_A(clr_s)>=254?dxRenderDeviceRender::Instance().ShaderTransform:dxRenderDeviceRender::Instance().m_SelectionShader,	clr_s);
    	m_SolidCone.Render	();
    }
