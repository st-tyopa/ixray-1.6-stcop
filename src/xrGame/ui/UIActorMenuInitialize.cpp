...
CUIActorMenu::CUIActorMenu()
{
	...
	if (READ_IF_EXISTS(pSettings, r_bool, "inventory", "rig", false))
	{
		ConstructCustom();
	}
	else
...
void CUIActorMenu::ConstructCustom()
{
	CUIXml								uiXml;
	uiXml.Load							(CONFIG_PATH, UI_PATH, "eft_actor_menu.xml");
	...
	m_pActorInventorySlotBackplate = new CUIWindow();
	CUIXmlInit::InitWindow(uiXml, "inventory_slot_wnd", 0, m_pActorInventorySlotBackplate);
	AttachChild(m_pActorInventorySlotBackplate);
	...
	for (u8 i = 1; i <= m_slot_count; ++i)
	{
		...
		m_pInvList[i] = new CUIDragDropListEx();
		m_pActorInventorySlotBackplate->AttachChild(m_pInvList[i]);
		CUIXmlInitGame::InitDragDropListEx(uiXml, "slot_dragdrop", 0, m_pInvList[i]);
		m_pInvList[i]->SetAutoDelete(true);
		...
		{
			m_pInvSlotHighlight[i] = new CUIStatic();
			m_pActorInventorySlotBackplate->AttachChild(m_pInvSlotHighlight[i]);
			CUIXmlInit::InitStatic(uiXml, "slot_highlight", 0, m_pInvSlotHighlight[i]);
			m_pInvSlotHighlight[i]->SetAutoDelete(true);
			m_pInvSlotHighlight[i]->Show(false);
		}
		...
		m_pInvSlotProgress[i] = new CUIProgressBar();
		m_pActorInventorySlotBackplate->AttachChild(m_pInvSlotProgress[i]);
		CUIXmlInit::InitProgressBar(uiXml, "slot_progress", 0, m_pInvSlotProgress[i]);
		m_pInvSlotProgress[i]->SetAutoDelete(true);
	}
	uiXml.SetLocalRoot(stored_root);
	...
	xml_init.InitTabControl(uiXml, "inv_tabs_panel", 0, m_pTabControl);
	AttachChild(m_pTabControl);
	
	//m_pTabControl->SetActiveTabByIndex(0);
	
	//CUIStackPanel* pInvStackPanel = UIHelper::CreateStackPanel(uiXml, "inv_tabs_panel", this, true);
	//UIHelper::Create3tButton(uiXml, "inv_tabs_panel:tab_inv_button", this);
	//UIHelper::Create3tButton(uiXml, "inv_tabs_panel:tab_stats_button", this);
	//pInvStackPanel->AttachChild();
	
	m_pActorInventoryScroll = UIHelper::CreateScrollView(uiXml, "inv_scroll", this);
	m_pActorInventoryScrollBackplate = new CUIWindow();
	CUIXmlInit::InitWindow(uiXml, "inv_scroll:back_plate", 0, m_pActorInventoryScrollBackplate);
	m_pActorInventoryScroll->AddWindow(m_pActorInventoryScrollBackplate, true);

	m_pInventoryRigLabel = UIHelper::CreateStatic(uiXml, "inv_scroll:rig_label", m_pActorInventoryScrollBackplate);
	m_pInventoryBagLabel = UIHelper::CreateStatic(uiXml, "inv_scroll:bag_label", m_pActorInventoryScrollBackplate);
	m_pInventoryBeltLabel = UIHelper::CreateStatic(uiXml, "inv_scroll:belt_label", m_pActorInventoryScrollBackplate);
	m_pInventoryRigList = UIHelperGame::CreateDragDropListEx(uiXml, "inv_scroll:dragdrop_rig", m_pActorInventoryScrollBackplate);
	m_pInventoryBagList = UIHelperGame::CreateDragDropListEx(uiXml, "inv_scroll:dragdrop_bag", m_pActorInventoryScrollBackplate);
	m_pInventoryBeltList = UIHelperGame::CreateDragDropListEx(uiXml, "inv_scroll:dragdrop_belt", m_pActorInventoryScrollBackplate);
	...
	for (u8 i = 0; i < rows; ++i)
	{
		for (u8 j = 0; j < cols; ++j)
		{
			if (i == 0 && j == 0)
			{
				m_belt_list_over[0] = UIHelper::CreateStatic(uiXml, "belt_list_over", m_pInventoryBeltLabel);
				pos = m_belt_list_over[0]->GetWndPos();
				dx = uiXml.ReadAttribFlt("belt_list_over", 0, "dx", 10.0f);
				dy = uiXml.ReadAttribFlt("belt_list_over", 0, "dy", 10.0f);
			}
			else
			{
				...
				m_belt_list_over[counter] = UIHelper::CreateStatic(uiXml, "belt_list_over", m_pInventoryBeltLabel);
				m_belt_list_over[counter]->SetWndPos(pos);
				counter++;
			}
		}
...
//-	AttachChild							(m_ItemInfo);
	...
	UIHelper::CreateFrameWindow(uiXml, "inspect_wnd", this, false);
