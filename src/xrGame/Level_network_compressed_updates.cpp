...
void CLevel::ProcessCompressedUpdate(NET_Packet& P, u8 const compress_type)
{
	...
	while (next_size)
	{
		if (compress_type & eto_ppmd_compression)
		{
			...
			uncompressed_packet.SetBufferSize
			(
				ppmd_trained_decompress
				(
					uncompressed_packet.GetBuffer(),
					sizeof(uncompressed_packet.GetBuffer()),
					P.GetBuffer() + P.r_tell(),
					next_size,
					m_trained_stream
				)
			);
		...
		else if (compress_type & eto_lzo_compression)
		{
			...
			uncompressed_packet.SetBufferSize(NET_PacketSizeLimit);
			lzo_decompress_dict(
				P.GetBuffer() + P.r_tell(),
				next_size,
				uncompressed_packet.GetBuffer(),
				(lzo_uint*)uncompressed_packet.GetBufferSize(),
		...
		VERIFY2(uncompressed_packet.GetBufferSize() <= NET_PacketSizeLimit,
