...
void MultipacketSender::SendPacket( const void* packet_data, u32 packet_sz, u32 flags, u32 timeout )
{
    ...
    if(     (buf->buffer.GetBufferSize() + packet_sz + sizeof(u16) >= NET_PacketSizeLimit)
        ||  (old_flags != new_flags)
...
void MultipacketSender::_FlushSendBuffer(u32 timeout, Buffer* buf)
{
    ...
    if (buf->buffer.GetBufferSize())
    {
        // compress data

        unsigned comp_sz = Compressor.compressed_size(buf->buffer.GetBufferSize());
        ...
        comp_sz = Compressor.Compress(packet_data + sizeof(MultipacketHeader), sizeof(packet_data) - sizeof(MultipacketHeader),
            buf->buffer.GetBuffer(), buf->buffer.GetBufferSize()
        );

        header->tag = NET_TAG_MERGED;
        header->unpacked_size = (u16)buf->buffer.GetBufferSize();
        ...
        if (strstr(Core.Params, "-dump_traffic"))
        {
            ...
            u16 sz = u16(buf->buffer.GetBufferSize());

            fwrite(&sz, sizeof(u16), 1, dump);
            fwrite(buf->buffer.GetBuffer(), buf->buffer.GetBufferSize(), 1, dump);
            fclose(dump);
        }

        // do send
        _SendTo_LL(packet_data, comp_sz + sizeof(MultipacketHeader), buf->last_flags, timeout);
        buf->buffer.SetBufferSize(0);
    }
}
